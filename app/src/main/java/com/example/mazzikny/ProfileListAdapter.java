...
public class ProfileListAdapter extends RecyclerView.Adapter<ProfileListAdapter.ExampleViewHolder> implements Filterable {
    ...
    public void onBindViewHolder(@NonNull ExampleViewHolder holder, int position) {
        ...
        holder.mTextView2.setText(currentItem.getUserInstrument());
        holder.mTextView3.setText(currentItem.getUserProf());
    ...
    public Filter examplefilter = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            else
            {
                ...
                {
                    if (item.getUserName().toLowerCase().contains(filterPattern) || item.getUserInstrument().toLowerCase().contains(filterPattern) || item.getUserProf().toLowerCase().contains(filterPattern))
                    {
